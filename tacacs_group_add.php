<?php
  ...
  if (!empty($_POST['submit']) && ($_POST['submit'] == 'yes')) {
    ...
    if(empty($_POST['groupname']) || (strlen($_POST['groupname']) >= 64)) {
      ...
      //save data to DB
      $query = "INSERT INTO tacgroup VALUES ('".$_POST['groupname']."', '');";
...
?>
          <form class="form-horizontal" action="/tacacs/group/add<?php if (!empty($_GET['return'])) { echo "?return=".$_GET['return']; } ?>" method="post">
            <legend>Group</legend>
