<?php
  ...
  if (!empty($_POST['submit']) && ($_POST['submit'] == 'yes')) {
    ...
    if (empty($_POST['groupname']) || ($_POST['groupname'] == '---')) {
      ...
      //save data to DB
      $query = "INSERT INTO radgroupcheck (groupname, attribute, op, value) VALUES ('".$_POST['groupname']."', '".$_POST['attribute']."', '".$_POST['op']."', '".$_POST['value']."' );";
...
?>
          ...
          <form class="form-horizontal" action="/radius/group/check/add<?php if (!empty($_GET['return'])) { echo "?return=".$_GET['return']; } ?>" method="post">
            <legend>For</legend>
